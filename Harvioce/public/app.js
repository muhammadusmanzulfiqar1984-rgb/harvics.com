...
outreachListEl?.addEventListener('click', async (e) => {
  ...
  try {
    ...
    if (action === 'send') {
      ...
      if (!resp.ok) {
        statusEl.textContent = `Send failed: ${data.error || resp.statusText}`;
      } else {
        statusEl.textContent = `Email sent to ${data.sentTo} ✓`;
      }
    }
...
const spamTestBtn = document.getElementById('spamTestBtn');
const spamTestAddress = document.getElementById('spamTestAddress');
const spamTestResult = document.getElementById('spamTestResult');

spamTestBtn?.addEventListener('click', async () => {
  const address = spamTestAddress?.value.trim();
  if (!address) {
    spamTestResult.textContent = 'Paste your mail-tester.com address first.';
    return;
  }
  spamTestBtn.textContent = 'Sending…';
  spamTestBtn.disabled = true;
  spamTestResult.textContent = '';
  try {
    const resp = await fetch('/api/outreach/spam-test', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ testAddress: address, subject: 'Harvics Trade — Outreach Test' })
    });
    const data = await resp.json();
    if (!resp.ok) {
      spamTestResult.innerHTML = `<span style="color:#e05050">Error: ${data.error}</span>`;
    } else {
      spamTestResult.innerHTML = `Sent! <a href="${data.resultUrl}" target="_blank" style="color:#C3A35E;font-weight:600">View your spam score →</a>`;
    }
  } catch (err) {
    spamTestResult.textContent = `Failed: ${err.message}`;
  } finally {
    spamTestBtn.textContent = 'Test Spam';
    spamTestBtn.disabled = false;
  }
});
/* ──────────────────────────────────────────────────────────────────── */
