        ...
        },
        {
          ...
          component: (
            <div className="p-6 space-y-6">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">AI Route Optimization</h3>
                  ...
                  <tbody>
                    {[{ route: 'Dubai North A', old: 148, opt: 112, stops: 18 }, { route: 'Dubai South B', old: 162, opt: 124, stops: 22 }, { route: 'Sharjah Loop', old: 95, opt: 78, stops: 14 }, { route: 'Abu Dhabi Central', old: 210, opt: 168, stops: 26 }].map((r, i) => (
                      <tr key={r.route} className={i % 2 === 0 ? 'bg-white' : 'bg-[#F5F5F7]'}>
                        <td className="px-4 py-3 font-semibold text-[#1D1D1F]">{r.route}</td>
                        <td className="px-4 py-3 text-right text-[#8E8E93] line-through">{r.old}</td>
                        <td className="px-4 py-3 text-right font-semibold text-[#1D1D1F]">{r.opt}</td>
                        <td className="px-4 py-3 text-right font-semibold text-[#1D1D1F]">-{r.old - r.opt} km</td>
        ...
        },
        {
          ...
          component: (
            <div className="p-6 space-y-4">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Route Performance Analytics</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
    ...
    },
    {
      ...
      tier3Screens: [
        ...
        },
        {
          ...
          component: (
            <div className="p-6 space-y-4">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Fleet Vehicle Status</h3>
                  ...
                  <tbody>
                    {[{ veh: 'DXB-TRK-001', driver: 'Hamdan Ali', km: 48200, service: '2026-04-15', status: 'On Route' }, { veh: 'DXB-TRK-002', driver: 'Majid Hassan', km: 62400, service: '2026-03-30', status: 'On Route' }, { veh: 'AUH-VAN-001', driver: 'Saeed Ibrahim', km: 31800, service: '2026-05-10', status: 'Available' }, { veh: 'SHJ-TRK-001', driver: '—', km: 78900, service: '2026-03-25', status: 'Maintenance' }].map((v, i) => (
                      <tr key={v.veh} className={i % 2 === 0 ? 'bg-white' : 'bg-[#F5F5F7]'}>
                        <td className="px-4 py-3 font-mono text-xs font-semibold text-[#1D1D1F]">{v.veh}</td>
                        ...
                        <td className="px-4 py-3 text-center"><span className={`px-2 py-1 text-xs font-bold ${v.status === 'On Route' ? 'bg-[#1D1D1F] text-white' : v.status === 'Maintenance' ? 'bg-[#6B1F2B] text-white' : 'bg-[#F5F5F7] text-[#8E8E93]'}`} style={{ borderRadius: 0 }}>{v.status}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        },
        {
          ...
          component: (
            <div className="p-6 space-y-4">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Live GPS — Vehicle Positions</h3>
              <div className="bg-[#F5F5F7] border-2 border-dashed border-[#E5E5EA]/40 h-56 flex items-center justify-center" style={{ borderRadius: 0 }}>
                <div className="text-center">
                  ...
                  <div className="font-semibold text-[#1D1D1F]">28 vehicles tracked live</div>
                  ...
                  <tbody>
                    {[{ veh: 'DXB-TRK-001', loc: 'Al Quoz Industrial, Dubai', speed: '42 km/h', time: '1 min ago' }, { veh: 'DXB-TRK-002', loc: 'Deira, Dubai', speed: '0 km/h', time: '2 min ago' }, { veh: 'AUH-VAN-001', loc: 'Hamdan St, Abu Dhabi', speed: '65 km/h', time: '1 min ago' }].map((g, i) => (
                      <tr key={g.veh} className={i % 2 === 0 ? 'bg-white' : 'bg-[#F5F5F7]'}>
                        <td className="px-4 py-3 font-mono text-xs font-semibold text-[#1D1D1F]">{g.veh}</td>
    ...
    },
    {
      ...
      tier3Screens: [
        ...
        },
        {
          ...
          component: (
            <div className="p-6 space-y-4">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Proof of Delivery</h3>
                  ...
                  <tbody>
                    {[{ id: 'DEL-2841', customer: 'Al Fardan Supermarket', driver: 'Hamdan Ali', time: '09:24', pod: 'Signed' }, { id: 'DEL-2840', customer: 'Carrefour Deira', driver: 'Majid Hassan', time: '10:05', pod: 'Signed' }, { id: 'DEL-2839', customer: 'Union Coop JBR', driver: 'Saeed Ibrahim', time: '11:30', pod: 'Pending' }, { id: 'DEL-2838', customer: 'Lulu Hypermarket', driver: 'Ahmed Nasser', time: '08:15', pod: 'Disputed' }].map((d, i) => (
                      <tr key={d.id} className={i % 2 === 0 ? 'bg-white' : 'bg-[#F5F5F7]'}>
                        ...
                        <td className="px-4 py-3 font-semibold text-[#1D1D1F]">{d.customer}</td>
                        <td className="px-4 py-3">{d.driver}</td>
                        <td className="px-4 py-3 text-xs">{d.time}</td>
                        <td className="px-4 py-3 text-center"><span className={`px-2 py-1 text-xs font-bold ${d.pod === 'Signed' ? 'bg-[#1D1D1F] text-white' : d.pod === 'Disputed' ? 'bg-[#6B1F2B] text-white' : 'bg-[#F5F5F7] text-[#8E8E93]'}`} style={{ borderRadius: 0 }}>{d.pod}</span></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        },
        {
          ...
          component: (
            <div className="p-6 space-y-6">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Delivery Performance Reports</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
    ...
    },
    {
      ...
      tier3Screens: [
        ...
        },
        {
          ...
          component: (
            <div className="p-6 space-y-4">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Return Reason Analysis</h3>
              <div className="space-y-3">
                {[{ reason: 'Product Damaged in Transit', count: 84, pct: 38 }, { reason: 'Wrong Product Delivered', count: 42, pct: 19 }, { reason: 'Near Expiry / Expired', count: 38, pct: 17 }, { reason: 'Customer Refused', count: 28, pct: 13 }, { reason: 'Overstock at Retailer', count: 18, pct: 8 }, { reason: 'Other', count: 11, pct: 5 }].map(r => (
                  <div key={r.reason} className="flex items-center gap-4">
                    <div className="w-48 text-sm text-[#1D1D1F] flex-shrink-0">{r.reason}</div>
                    ...
                    <div className="w-12 text-right text-sm font-semibold text-[#1D1D1F]">{r.count}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        },
        {
          ...
          component: (
            <div className="p-6 space-y-6">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Returns Trend Analytics</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
      ...
      tier3Screens: [
        {
          ...
          component: (
            <div className="p-6 space-y-6">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Retailer Coverage Heatmap</h3>
              <div className="bg-[#F5F5F7] h-48 flex items-center justify-center border-2 border-dashed border-[#E5E5EA]/30" style={{ borderRadius: 0 }}>
                <div className="text-center"><div className="text-3xl mb-2">️</div><div className="font-semibold text-[#1D1D1F]">Interactive Heatmap</div><div className="text-sm text-[#8E8E93]">972 tracked retailers across 23 active routes</div></div>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[{ label: 'High Density', count: 12, color: 'bg-[#F5F5F7] text-[#1D1D1F]' }, { label: 'Medium', count: 28, color: 'bg-[#F5F5F7] text-[#1D1D1F]' }, { label: 'Low', count: 15, color: 'bg-[#F5F5F7] text-[#1D1D1F]' }, { label: 'No Coverage', count: 8, color: 'bg-[#F5F5F7] text-[#1D1D1F]' }].map(z => (
                  <div key={z.label} className="border border-[#E5E5EA]/30 p-4" style={{ borderRadius: 0 }}>
                    <div className="text-sm text-[#8E8E93]">{z.label}</div>
                    <div className="text-2xl font-semibold text-[#1D1D1F]">{z.count} zones</div>
      ...
      tier3Screens: [
        {
          ...
          component: (
            <div className="p-6 space-y-4">
              <h3 className="text-xl font-semibold text-[#1D1D1F] mb-4">Active Trade Flows</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {[{ id: 'TF-001', type: 'Import', origin: 'Shanghai, CN', dest: 'Dubai, UAE', status: 'In Transit', progress: 65 }, { id: 'TF-002', type: 'Export', origin: 'Dubai, UAE', dest: 'Frankfurt, DE', status: 'Customs', progress: 45 }, { id: 'TF-003', type: 'Import', origin: 'Bangkok, TH', dest: 'Karachi, PK', status: 'Loading', progress: 20 }].map((f, i) => (
                      <tr key={f.id} className={i % 2 === 0 ? 'bg-white' : 'bg-[#F5F5F7]'}>
                        <td className="px-4 py-3 font-semibold text-[#1D1D1F]">{f.id}</td>
                        <td className="px-4 py-3"><span className={`px-2 py-1 text-xs font-bold ${f.type === 'Import' ? 'bg-[#F5F5F7] text-[#1D1D1F]' : 'bg-[#F5F5F7] text-[#1D1D1F]'}`} style={{ borderRadius: 0 }}>{f.type}</span></td>
                        <td className="px-4 py-3">{f.origin}</td>
                        <td className="px-4 py-3">{f.dest}</td>
                        <td className="px-4 py-3 text-center"><span className="px-2 py-1 text-xs font-bold bg-[#F5F5F7] text-[#1D1D1F]" style={{ borderRadius: 0 }}>{f.status}</span></td>
