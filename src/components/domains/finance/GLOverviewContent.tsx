...
export default function GLOverviewContent({ persona, locale }: { persona: string; locale: string }) {
  const { data, source, lastUpdated, loading } = useDomainData('finance')

  if (loading) return <div className="p-8 text-sm text-[#8E8E93]">Loading…</div>
  if (!data) return <div className="p-8 text-sm text-[#8E8E93]">No finance data yet — post invoices in Finance OS.</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Total Revenue" value={fmtM(data.revenue || 0)} sparkline={data.revenueByMonth}
        ...
        />
        <KPICard label="Net Profit" value={fmtM(data.netProfit || 0)} sparkline={data.revenueByMonth?.map((v: number, i: number) => v - (data.expenseByMonth?.[i] || 0))}
          icon={<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>}
        />
        <KPICard label="Margin" value={margin} suffix="%"
        ...
        />
        <KPICard label="A/R Balance" value={fmtM(data.arBalance || 0)}
        ...
        <Card title="Expense Breakdown">
          <div className="p-5 flex flex-col items-center gap-4">
            <DonutChart
              ...
              centerValue={fmtM(data.expenses || 0)}
            />
        ...
        <Card>
          <div className="p-5 space-y-4">
            ...
            <div className="space-y-3">
              {[
                { label: 'Cash On Hand', value: data.cashOnHand || 0, color: '#34C759' },
                { label: 'A/R Balance', value: data.arBalance || 0, color: '#007AFF' },
                { label: 'A/P Balance', value: data.apBalance || 0, color: '#FF9500' },
                { label: 'Pending Payments', value: data.pendingPayments || 0, color: '#FF3B30' },
              ].map((item) => (
        ...
        <Card className="md:col-span-2">
          <div className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { label: 'Gross Margin', value: data.revenue > 0 ? `${((data.grossProfit / data.revenue) * 100).toFixed(1)}%` : '—', sub: 'Gross Profit / Revenue' },
                { label: 'Net Margin', value: `${margin}%`, sub: 'Net Profit / Revenue' },
                { label: 'ROI', value: '—', sub: 'Requires invested capital data' },
                { label: 'Working Capital', value: fmtM((data.cashOnHand || 0) + (data.arBalance || 0) - (data.apBalance || 0)), sub: 'Cash + A/R − A/P' },
              ].map((r) => (
