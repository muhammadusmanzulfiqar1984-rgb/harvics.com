...
export default function StockOverviewContent({ persona, locale }: { persona: string; locale: string }) {
  const { data, source, lastUpdated, loading } = useDomainData('inventory')
  const [filter, setFilter] = useState<'All' | 'Low'>('All')

  if (loading) return <div className="p-8 text-sm text-[#8E8E93]">Loading…</div>
  if (!data) return <div className="p-8 text-sm text-[#8E8E93]">No inventory data yet — add SKUs in Inventory OS.</div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-[#8E8E93] mt-0.5">Real-time inventory across {data.warehouseCount || 0} warehouses</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard label="Total Stock Value" value={fmtM(data.totalValue || 0)} sparkline={data.stockTrend}
        ...
        />
        <KPICard label="Active SKUs" value={data.totalSkus || 0}
        ...
        />
        <KPICard label="Low Stock Alerts" value={data.lowStock || 0}
        ...
        />
        <KPICard label="Turnover Days" value={data.turnoverDays ?? '—'}
        ...
        <Card title="Value by Category">
          <div className="p-5 flex flex-col items-center gap-4">
            <DonutChart
              ...
              size={120}
              centerLabel="Total"
              centerValue={fmtM(data.totalValue || 0)}
            />
        ...
        <div className="flex gap-1.5">
          {(['All', 'Low'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {f === 'Low' ? `⚠ Low Stock (${data.lowStock || 0})` : 'All SKUs'}
            </button>
          ))}
        </div>
      </div>
