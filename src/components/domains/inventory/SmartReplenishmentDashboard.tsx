  ...
  if (loading) {
    return (
      <div className="p-8 flex flex-col items-center justify-center text-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#E5E5EA] mb-4"></div>
        <h3 className="text-lg font-semibold text-[#1D1D1F]">Sovereign Architect Analysis...</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {opportunities.map((opp, idx) => (
          <div key={idx} className="bg-white border border-[#E5E5EA]/30 rounded-lg shadow-md hover:shadow-xl transition-shadow overflow-hidden group">
            <div className="p-4 border-b border-gray-100 bg-[#F5F5F7] flex justify-between items-center">
              <span className={`px-2 py-1 text-xs font-bold uppercase rounded ${
                opp.status === 'Critical' ? 'bg-[#F5F5F7] text-[#1D1D1F]' : 
                opp.status === 'High' ? 'bg-[#F5F5F7] text-[#1D1D1F]' : 'bg-[#F5F5F7] text-[#1D1D1F]'
              }`}>
                {opp.status} Void
              </span>
            ...
            <div className="p-6">
              <h3 className="text-lg font-semibold text-[#1D1D1F] mb-1 group-hover:text-[#C3A35E] transition-colors">
                {opp.sku}
              </h3>
              ...
              <div className="flex items-end justify-between mb-4">
                <div>
                  ...
                  <p className="text-lg font-semibold text-[#1D1D1F]">{opp.margin}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-gray-400 uppercase">Strategy</p>
                  <p className="text-xs font-semibold text-[#1D1D1F] bg-[#F5F5F7] px-2 py-1 rounded">
                    {opp.strategy}
                  </p>
                </div>
              </div>
      ...
      <div className="bg-[#F5F5F7] border border-[#E5E5EA] rounded-lg p-4 flex items-start gap-4">
        ...
        <div>
          <h4 className="font-semibold text-[#1D1D1F]">Sovereign Architect Insight</h4>
