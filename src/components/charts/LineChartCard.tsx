...
export default function LineChartCard({ title, data, dataKeys, colors = [CT.burgundy, '#0F766E'], height = 300, className = '' }: LineChartCardProps) {
  return (
    <div className={`relative overflow-hidden rounded-xl p-5 ${className}`}
      ...
      <ResponsiveContainer width="100%" height={height}>
        <LineChart data={data} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
          ...
          <Tooltip content={<Tip />} cursor={{ stroke: CT.gold, strokeWidth: 1, strokeOpacity: 0.4 }} />
          <Legend iconType="plainline" iconSize={16} formatter={(v: string) => <span style={{ color: CT.muted, fontSize: 11, fontWeight: 600 }}>{v}</span>} />
          {dataKeys.map((key, i) => (
            <Line key={key} type="monotone" dataKey={key} stroke={colors[i % colors.length]} strokeWidth={2}
              dot={false} activeDot={{ r: 4, stroke: colors[i % colors.length], strokeWidth: 2, fill: CT.bg }} />
          ))}
        </LineChart>
      </ResponsiveContainer>
    </div>
