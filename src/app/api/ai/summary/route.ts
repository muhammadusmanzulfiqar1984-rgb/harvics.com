...
export async function POST(req: NextRequest) {
  try {
    ...
    const summaryMessages = [
      { role: 'system' as const, content: 'You are a VAT expense analyst. Provide a brief 2-3 sentence summary of the expense report highlighting top categories and VAT implications.' },
      { role: 'user' as const, content: `Total: ${currency}${totalAmount}. Expenses: ${JSON.stringify(expenses?.slice(0, 10))}` },
    ]

    // 1. OpenAI
    const openAISummary = await callOpenAI(summaryMessages, { maxTokens: 200 })
    if (openAISummary) return NextResponse.json({ summary: openAISummary })

    // 2. NVIDIA
    const nvidiaSummary = await callNvidia(summaryMessages, { maxTokens: 200 })
    if (nvidiaSummary) return NextResponse.json({ summary: nvidiaSummary })

    // 3. Cloudflare Workers AI
    if (ai && typeof (ai as { run?: unknown }).run === 'function') {
      const cfAI = ai as { run: (model: string, opts: unknown) => Promise<{ response?: string }> }
      const result = await cfAI.run('@cf/meta/llama-3-8b-instruct', { messages: summaryMessages, max_tokens: 200 })
