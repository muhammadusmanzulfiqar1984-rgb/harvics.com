...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Deepgram Nova (primary when DEEPGRAM_API_KEY is set)
    const deepgramText = await transcribeWithDeepgram(audioFile, 'audio.webm')
    if (deepgramText) return NextResponse.json({ text: deepgramText, provider: 'deepgram' })

    // 2. Cloudflare AI Whisper (no extra cost, uses CF binding)
    ...
    if (ai && typeof (ai as { run?: unknown }).run === 'function') {
      ...
      if (result?.text) return NextResponse.json({ text: result.text, provider: 'cloudflare' })
    }

    // 3. NVIDIA Parakeet ASR fallback
    const nvidiaText = await transcribeWithNvidia(audioFile, 'audio.webm')
    if (nvidiaText) return NextResponse.json({ text: nvidiaText, provider: 'nvidia' })

    // 4. OpenAI Whisper-1 fallback
    const text = await transcribeWithOpenAI(audioFile, 'audio.webm')
    return NextResponse.json({ text, provider: text ? 'openai' : undefined })
  } catch {
    return NextResponse.json({ text: '' })
  }
}
