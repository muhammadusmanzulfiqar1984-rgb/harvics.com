    ...
    <main className="min-h-screen pt-[136px]" style={{ background: '#ffffff' }}>
      ...
      <section className="relative bg-harvics-burgundy py-20 px-4 border-b border-harvics-gold/40 overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto text-center relative z-10">
          <div className="text-xs text-harvics-gold font-bold uppercase tracking-[0.2em] mb-3">{t('joinUs')}</div>
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-16">
        <h2 className="text-2xl font-semibold text-harvics-burgundy mb-8 text-center">{t('openPositionsByDept')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {departments.map((dept) => (
            <div
              key={dept.key}
              className="bg-white border border-harvics-gold/20 p-6 hover:border-harvics-gold transition-colors group cursor-pointer"
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-base font-semibold text-harvics-burgundy">{t(`departments.${dept.key}.name`)}</h3>
                    <span className="text-xs font-bold text-harvics-gold bg-harvics-gold/10 px-3 py-1" style={{ borderRadius: 0 }}>{dept.count} {t('roles')}</span>
                  </div>
                  <p className="text-sm text-harvics-burgundy/55 leading-relaxed">{t(`departments.${dept.key}.desc`)}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section className="bg-white border-t border-b border-harvics-gold/20">
        <div className="max-w-[1200px] mx-auto px-4 py-16">
          <h2 className="text-2xl font-semibold text-harvics-burgundy mb-8 text-center">{t('whyHarvics')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {benefits.map((b) => (
              <div key={b.key} className="p-6 border border-harvics-gold/15 bg-white" style={{ borderRadius: 0 }}>
                <span className="text-2xl">{b.icon}</span>
                <h3 className="text-base font-semibold text-harvics-burgundy mt-4 mb-2">{t(`benefits.${b.key}.title`)}</h3>
                <p className="text-sm text-harvics-burgundy/55 leading-relaxed">{t(`benefits.${b.key}.desc`)}</p>
              </div>
            ))}
          </div>
        </div>
