    ...
    <main className="min-h-screen pt-[136px]" style={{ background: '#ffffff' }}>
      ...
      <section className="relative bg-harvics-burgundy py-20 px-4 border-b border-harvics-gold/40 overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto text-center relative z-10">
          <div className="text-xs text-harvics-gold font-bold uppercase tracking-[0.2em] mb-3">Trade Calendar</div>
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {seasons.map((s) => (
            <div key={s.title} className="bg-white border border-harvics-gold/15 p-8">
              <div className="flex items-center gap-3 mb-4">
                <span className="text-3xl">{s.icon}</span>
                <div>
                  <h3 className="text-lg font-semibold text-harvics-burgundy">{s.title}</h3>
                  <span className="text-xs text-harvics-gold font-bold uppercase tracking-wider">{s.period}</span>
                </div>
              </div>
              <ul className="space-y-3">
                {s.offers.map((offer) => (
                  <li key={offer} className="flex items-start gap-2">
                    <span className="text-harvics-gold mt-0.5 flex-shrink-0">→</span>
                    <span className="text-sm text-harvics-burgundy/55 leading-relaxed">{offer}</span>
                  </li>
                ))}
              </ul>
            </div>
