    ...
    <main className="min-h-screen bg-gradient-to-b from-[#F5F1E8] to-white">
      <section className="max-w-5xl mx-auto px-6 pt-24 pb-16">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#1A0505] to-[#C3A35E] flex items-center justify-center text-white text-2xl font-bold shadow-lg">E</div>
          <div>
            <span className="inline-block px-3 py-0.5 text-xs font-semibold rounded-full bg-amber-100 text-amber-800 mb-1">Beta</span>
            <h1 className="text-3xl font-bold text-[#1A0505]">Harvics Event OS</h1>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-16">
          ...
          <Link
            href="/en/apps"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border border-[#1A0505]/20 text-[#1A0505] font-semibold hover:bg-[#1A0505]/5 transition"
          >
            ← Back to Apps
          </Link>
        </div>

        <h2 className="text-xl font-bold text-[#1A0505] mb-6">Features</h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
          {FEATURES.map((f) => (
            <div key={f.title} className="p-6 rounded-2xl bg-white border border-gray-100 shadow-sm">
              <div className="text-3xl mb-3">{f.icon}</div>
              <h3 className="font-semibold text-[#1A0505] mb-1">{f.title}</h3>
              <p className="text-sm text-gray-600">{f.desc}</p>
            </div>
          ))}
        </div>

        <div className="p-8 rounded-2xl bg-[#1A0505]/5 border border-[#1A0505]/10">
          <h3 className="font-bold text-[#1A0505] mb-2">Tech Stack</h3>
