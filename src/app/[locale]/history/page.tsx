    ...
    <main className="min-h-screen pt-[136px]" style={{ background: '#ffffff' }}>
      ...
      <section className="relative bg-harvics-burgundy py-20 px-4 border-b border-harvics-gold/40 overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto text-center relative z-10">
          <div className="text-xs text-harvics-gold font-bold uppercase tracking-[0.2em] mb-3">Since 2019</div>
          ...
          <div className="space-y-8">
            {milestones.map((m, idx) => (
              <div key={m.year} className="relative flex gap-6">
                {/* Year circle */}
                <div className="flex-shrink-0 w-20 h-20 bg-harvics-burgundy border-2 border-harvics-gold/40 flex items-center justify-center z-10" style={{ borderRadius: 0 }}>
                  <span className="text-harvics-gold font-bold text-sm">{m.year}</span>
                </div>

                {/* Content card */}
                <div className="flex-1 bg-white border border-harvics-gold/20 p-6 hover:border-harvics-gold transition-colors" style={{ borderRadius: 0, boxShadow: 'none' }}>
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-lg font-semibold text-harvics-burgundy">{m.title}</h3>
                    <span className="text-xs font-bold text-harvics-gold bg-harvics-gold/10 px-3 py-1" style={{ borderRadius: 0 }}>{m.highlight}</span>
                  </div>
                  <p className="text-sm text-harvics-burgundy/60 leading-relaxed">{m.description}</p>
                </div>
              </div>
            ))}
          </div>
