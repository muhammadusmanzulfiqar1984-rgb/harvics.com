    ...
    <main className="min-h-screen pt-[136px]" style={{ background: '#ffffff' }}>
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {contacts.map((c) => (
            <div key={c.title} className="bg-white border border-harvics-gold/15 p-8">
              <div className="text-3xl mb-4">{c.icon}</div>
              <h3 className="text-lg font-semibold text-harvics-burgundy mb-2">{c.title}</h3>
              <p className="text-sm text-harvics-burgundy/55 leading-relaxed mb-4">{c.desc}</p>
              <a href={`mailto:${c.email}`} className="text-sm font-bold text-harvics-gold hover:text-harvics-burgundy transition-colors">
                {c.email}
              </a>
            </div>
          ))}
      ...
      <section className="bg-white border-t border-harvics-gold/20">
        <div className="max-w-[1200px] mx-auto px-4 py-16">
          <h2 className="text-2xl font-semibold text-harvics-burgundy mb-8 text-center">Global Headquarters</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-[900px] mx-auto">
            {[
              ...
              <div key={office.city} className="text-center p-6 border border-harvics-gold/15">
                <div className="text-lg font-bold text-harvics-burgundy mb-1">{office.city}</div>
                <div className="text-xs text-harvics-gold font-semibold uppercase tracking-wider mb-3">{office.type}</div>
                <a href={`mailto:${office.email}`} className="text-sm text-harvics-burgundy/55 hover:text-harvics-gold transition-colors">{office.email}</a>
              </div>
            ))}
          </div>
        </div>
