...
export default async function ElectronicsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-white pt-[136px]">
      ...
      {/* Hero */}
      <section className="relative bg-[#3D1212] py-16 px-4 border-b border-[#C3A35E]/30 overflow-hidden">
        ...
        <div className="max-w-[1100px] mx-auto relative z-10">
          <div className="flex items-center gap-2 text-[10px] text-white/30 mb-5 tracking-[0.2em] uppercase">
            ...
            <span>/</span>
            <span className="text-[#C3A35E]">Consumer Electronics</span>
          </div>
          <div className="flex items-start justify-between gap-6">
            <div>
              <div className="w-10 h-10 border border-[#C3A35E]/30 flex items-center justify-center mb-5">
                <span className="text-[11px] font-bold text-[#C3A35E]/70 tracking-widest">CE</span>
              </div>
            ...
            <div className="hidden md:flex flex-col items-end gap-1 shrink-0">
              <div className="text-2xl font-bold text-[#C3A35E]" style={{ letterSpacing: '-0.03em' }}>{products.length}+</div>
              <div className="text-[10px] text-white/30 uppercase tracking-[0.18em]">Products</div>
            </div>
          </div>
          <div className="mt-8 inline-flex items-center gap-2 bg-[#C3A35E]/10 border border-[#C3A35E]/20 px-4 py-2.5">
            <span className="text-[10px] font-bold text-[#C3A35E] uppercase tracking-wider">Powered by</span>
            ...
            return (
              <div key={sub}>
                <div className="flex items-center justify-between mb-7">
                  <div>
                    <h2 className="text-lg font-bold text-[#3D1212]" style={{ letterSpacing: '-0.02em' }}>{sub}</h2>
                    <span className="text-[10px] text-[#C3A35E] font-bold uppercase tracking-wider">{subProducts.length} products</span>
                  </div>
                </div>
                  ...
                  {subProducts.map(p => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-[9px] font-bold text-[#C3A35E] bg-[#C3A35E]/10 px-2 py-0.5 uppercase tracking-wider">CE</span>
                        {p.verified && <span className="text-[9px] font-bold text-emerald-600">✓ Verified</span>}
                      </div>
                      <h3 className="text-sm font-semibold text-[#3D1212] mb-4 leading-snug group-hover:text-[#C3A35E] transition-colors min-h-[2.5rem]">
                        {p.name}
                      </h3>
                      <div className="space-y-1.5 text-[11px] text-[#3D1212]/40 mb-4">
                        <div className="flex justify-between">
                          <span>Origin</span>
                          <span className="font-semibold text-[#3D1212]/60">{p.origin}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>MOQ</span>
                          <span className="font-semibold text-[#3D1212]/60">{p.moq}</span>
                        </div>
                      </div>
                      <div className="pt-3 border-t border-[#C3A35E]/10 flex items-center justify-between">
                        ...
                        <span className="text-[9px] font-bold text-[#3D1212]/25 uppercase tracking-wider group-hover:text-[#C3A35E] transition-colors">RFQ →</span>
                      </div>
                    </Link>
                  ))}
                </div>
      ...
      {/* B2B CTA */}
      <section className="bg-[#3D1212] border-t border-[#C3A35E]/20">
        <div className="max-w-[1100px] mx-auto px-4 py-14 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <Link href={`/${locale}/harvictrade/rfq?category=Consumer+Electronics`}
            className="shrink-0 px-8 py-3 bg-[#C3A35E] text-[#3D1212] text-xs font-bold uppercase tracking-[0.18em] hover:bg-[#d4b46e] transition-colors">
            Submit Bulk RFQ
          </Link>
        </div>
      </section>
