...
export default async function SellPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <main className="min-h-screen pt-[136px]" style={{ background: '#ffffff' }}>
      {/* Hero */}
      <section className="relative bg-gradient-to-br from-harvics-burgundy via-[#5a1a24] to-[#4a1520] py-24 px-4 overflow-hidden">
        ...
        <div className="max-w-[1200px] mx-auto text-center relative z-10">
          <span className="inline-block text-xs font-bold text-harvics-gold uppercase tracking-[0.25em] mb-5 border border-harvics-gold/30 px-4 py-1.5">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-6" style={{ letterSpacing: '-0.03em' }}>
            ...
            <span className="text-harvics-gold">From Day One.</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href={`/${locale}/harvictrade/register`}
              className="px-8 py-4 bg-harvics-gold text-harvics-burgundy font-bold text-sm uppercase tracking-wider hover:bg-[#d4b46e] transition-colors">
              Register as Supplier
            </Link>
            <Link href={`/${locale}/contact`}
              className="px-8 py-4 border border-harvics-gold/50 text-harvics-gold font-bold text-sm uppercase tracking-wider hover:bg-harvics-gold/10 transition-colors">
              Talk to Sales
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-16">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-harvics-burgundy" style={{ letterSpacing: '-0.02em' }}>Why Sell on HarvicTrade?</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {benefits.map((b) => (
            <div key={b.title} className="bg-white border border-harvics-gold/15 p-8">
              <div className="text-3xl mb-4">{b.icon}</div>
              <h3 className="text-lg font-bold text-harvics-burgundy mb-2">{b.title}</h3>
              <p className="text-sm text-harvics-burgundy/50 leading-relaxed">{b.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Pricing Tiers */}
      <section className="bg-white border-t border-harvics-gold/15 py-16 px-4">
        <div className="max-w-[1200px] mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-harvics-burgundy" style={{ letterSpacing: '-0.02em' }}>Seller Plans</h2>
            <p className="text-harvics-burgundy/50 mt-2">Start free. Scale when you are ready.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-[1000px] mx-auto">
            {tiers.map((tier) => (
              <div key={tier.name} className={`p-8 flex flex-col ${tier.highlight ? 'bg-harvics-burgundy text-white border-2 border-harvics-gold' : 'bg-white border border-harvics-gold/15'}`}>
                <div className="mb-6">
                  <h3 className={`text-lg font-bold mb-1 ${tier.highlight ? 'text-harvics-gold' : 'text-harvics-burgundy'}`}>{tier.name}</h3>
                  <div className={`text-3xl font-bold ${tier.highlight ? 'text-white' : 'text-harvics-burgundy'}`}>{tier.price}</div>
                </div>
                <ul className="space-y-3 mb-8 flex-1">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-start gap-2 text-sm">
                      <span className={tier.highlight ? 'text-harvics-gold' : 'text-harvics-gold'}>✓</span>
                      <span className={tier.highlight ? 'text-white/80' : 'text-harvics-burgundy/60'}>{f}</span>
                    </li>
                  ))}
                </ul>
                <button className={`w-full py-3 font-bold text-sm uppercase tracking-wider transition-colors ${tier.highlight ? 'bg-harvics-gold text-harvics-burgundy hover:bg-[#d4b46e]' : 'bg-harvics-burgundy text-white hover:bg-[#5a1a24]'}`}>
                  {tier.cta}
                </button>
              </div>
            ))}
      ...
      <section className="bg-harvics-burgundy border-t border-harvics-gold/30">
        <div className="max-w-[1200px] mx-auto px-4 py-14 text-center">
          ...
          <Link href={`/${locale}/harvictrade/register`}
            className="inline-block px-10 py-4 bg-harvics-gold text-harvics-burgundy font-bold text-sm uppercase tracking-wider hover:bg-[#d4b46e] transition-colors">
            Start Selling Today
          </Link>
        </div>
      </section>
