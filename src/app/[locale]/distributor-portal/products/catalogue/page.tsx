...
export default function ProductCatalogue() {
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-2xl font-bold text-harvics-gold">{t('title')}</h1>

      {/* Filters */}
      <div className="bg-white p-4 border border-black200 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label className="block text-sm font-semibold text-harvics-gold/90 mb-2">{t('category')}</label>
            <select
          ...
          <div>
            <label className="block text-sm font-semibold text-harvics-gold/90 mb-2">{tCommon('country')}</label>
            <select
          ...
          <div>
            <label className="block text-sm font-semibold text-harvics-gold/90 mb-2">{t('channel')}</label>
            <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {products.map((product) => (
          <div key={product.sku} className="bg-white border border-black200 shadow-sm overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div className="p-4">
              <div className="text-xs text-harvics-gold/90 mb-1">{product.sku}</div>
              <h3 className="font-semibold text-harvics-gold/90 mb-2">{product.name}</h3>
              <div className="text-sm text-harvics-gold/90 mb-2">Pack Size: {product.packSize}</div>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <div className="text-xs text-harvics-gold/90">MRP</div>
                  <div className="text-lg font-bold text-harvics-gold/90">${product.mrp.toFixed(2)}</div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-harvics-gold/90">Distributor Price</div>
                  <div className="text-lg font-bold text-white">${product.distributorPrice.toFixed(2)}</div>
                </div>
              </div>
              <div className="flex space-x-2">
                <button className="flex-1 bg-white text-harvics-gold/90 px-4 py-2 font-semibold hover:bg-white transition-colors">
                  {t('details')}
                </button>
