...
/**
 * Document document — Module #52 (SAP-style).
 */
...
const NEXT: Record<string, string[]> = {"Draft":["Active","Archived"],"Active":["Signed","Expired","Archived"],"Signed":["Expired","Archived"],"Expired":["Archived"],"Archived":[]}

export default function Module52DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/v2/documents/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  return (
    <HarvicsOSShell
      ...
      subtitle="Module #52 — SAP+ vault document"
      activeDomain="platform"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Document Vault', href: '/os/document-vault' },
        { label: doc?.title || 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/document-vault`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
