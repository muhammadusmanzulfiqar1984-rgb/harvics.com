...
export default function Module67DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave6/wallets/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  return (
    <HarvicsOSShell
      title={doc ? String(doc.ownerId) : 'Document'}
      subtitle="Module #67 — SAP+ HPay wallet document"
      activeDomain="hpay"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'HPay Wallet', href: '/os/hpay-wallet' },
        { label: doc ? String(doc.ownerId) : 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/hpay-wallet`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← HPay Wallet workspace
        </Link>
        ...
        {!loading && doc ? (
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
              ['Owner', (doc.ownerType || '') + '/' + (doc.ownerId || '')],
              ['Currency', doc.currency ?? '—'],
              ['Balance', doc.balance ?? '—'],
              ['Status', doc.status ?? '—'],
              ['Label', doc.label ?? '—'],
              ['Txns', (doc.transactions || []).length]
              ].map(([k, v]) => (
