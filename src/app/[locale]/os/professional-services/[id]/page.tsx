...
/**
 * Engagement document — Module #47 (SAP-style).
 */
...
const NEXT: Record<string, string[]> = {"Active":["OnHold","Completed","Cancelled"],"OnHold":["Active","Cancelled"],"Completed":[],"Cancelled":[]}

export default function Module47DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave5/engagements/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  return (
    <HarvicsOSShell
      title={doc?.code || 'Engagement'}
      subtitle="Module #47 — SAP+ engagement document"
      activeDomain="service"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Professional Services', href: '/os/professional-services' },
        { label: doc?.code || 'Engagement' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/professional-services`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
        ...
        {!loading && doc ? (
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              ['Spent', doc.spent ?? '—'],
              ['Manager', doc.manager ?? '—'],
              ['Start', doc.startDate ? new Date(doc.startDate).toLocaleString() : '—'],
              ['End', doc.endDate ? new Date(doc.endDate).toLocaleString() : '—']
              ].map(([k, v]) => (
