...
export default function P(){
  ...
  <Grid cols={2}>
    ...
    <Panel title={`ENGAGEMENTS (${rows.length})`} full>
      {rows.map(r=>{const t=time[r.id]||{employeeId:'',date:new Date().toISOString().slice(0,10),hours:0,rate:0,description:'',billable:true};const burn=r.budget?(r.spent/r.budget*100):0;return <div key={r.id} style={{padding:10,marginBottom:8,border:'1px solid #6B1F2B22',background:'#fafafa'}}>
        ...
        <div style={{fontSize:11,marginTop:2,color:'#666'}}>Budget: <b style={{color:'#6B1F2B'}}>${r.budget.toLocaleString()}</b> · Spent: <b style={{color:burn>90?'#B71C1C':'#6B1F2B'}}>${r.spent.toLocaleString()} ({burn.toFixed(0)}%)</b></div>
        <div style={{marginTop:8,display:'grid',gridTemplateColumns:'1fr 110px 60px 70px 2fr auto',gap:4}}>
          ...
          <button onClick={()=>logTime(r.id)} style={{padding:'4px 12px',background:'#6B1F2B',color:'#fff',border:0,cursor:'pointer',fontSize:11,fontWeight:700}}>LOG</button>
        </div>
        {r.timeEntries.length>0 && <Tbl head={['DATE','EMP','HRS','RATE','AMT','DESC']}>{r.timeEntries.map((e:any)=><tr key={e.id}><td style={{...td,fontSize:11}}>{new Date(e.date).toLocaleDateString()}</td><td style={td}>{e.employeeId}</td><td style={td}>{e.hours}</td><td style={td}>${e.rate}</td><td style={{...td,fontWeight:700,color:'#6B1F2B'}}>${e.amount}</td><td style={{...td,fontSize:11}}>{e.description||'—'}</td></tr>)}</Tbl>}
      </div>})}
    </Panel>
  </Grid></Page>
}
