...
export default function Module59DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave6/feed/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  return (
    <HarvicsOSShell
      title={doc ? String(doc.authorName) : 'Document'}
      subtitle="Module #59 — SAP+ feed post document"
      activeDomain="feed"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Feed', href: '/os/feed' },
        { label: doc ? String(doc.authorName) : 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/feed`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← Feed workspace
        </Link>
