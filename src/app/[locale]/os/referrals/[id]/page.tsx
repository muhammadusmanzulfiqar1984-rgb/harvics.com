...
export default function Module68DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave6/referrals/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  const setStatus = async (status: string) => {
    try {
      setError(''); setMessage('')
      await api(`/api/wave6/referrals/${doc.referralCode}/status`, { method: 'POST', body: JSON.stringify({ status }) })
  ...
  return (
    <HarvicsOSShell
      title={doc ? String(doc.referralCode) : 'Document'}
      subtitle="Module #68 — SAP+ referral document"
      activeDomain="referrals"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Referrals', href: '/os/referrals' },
        { label: doc ? String(doc.referralCode) : 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/referrals`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← Referrals workspace
        </Link>
        ...
        {!loading && doc ? (
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              ['Referrer', doc.referrerName ?? '—'],
              ['Referee', doc.refereeEmail ?? '—'],
              ['Reward', (doc.rewardCurrency || '') + ' ' + (doc.rewardAmount ?? '—')],
              ['Status', doc.status ?? '—'],
              ['Signed', doc.signedAt ? new Date(doc.signedAt).toLocaleString() : '—'],
              ['Paid', doc.paidAt ? new Date(doc.paidAt).toLocaleString() : '—']
              ].map(([k, v]) => (
            ...
            <div className="flex flex-wrap gap-2 border border-harvics-burgundy/15 bg-harvics-cream/40 p-4">
              ...
              {['Signed','Qualified','Paid','Expired'].map((s) => (
