...
export default function DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave3/hr/leave/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  const setStatus = async (status: string, path?: string, body?: any) => {
    try {
      setError('')
      setMessage('')
      const endpoint = path || `/api/wave3/hr/leave/${id}/status`
  ...
  const title = doc ? (doc.leaveType || 'Leave') : 'Document'

  return (
    <HarvicsOSShell
      title={typeof title === 'string' ? title : String(title)}
      subtitle="Module #29 — SAP+ leave document"
      activeDomain="hr"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'HR', href: '/os/hr' },
          ...
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                ['Days', doc.days],
                ['Start', doc.startDate ? new Date(doc.startDate).toLocaleDateString() : '—'],
                ['End', doc.endDate ? new Date(doc.endDate).toLocaleDateString() : '—'],
                ['Reason', doc.reason || '—'],
            ...
            {doc.status === 'Pending' ? (
              <div className="flex flex-wrap gap-2 border border-harvics-burgundy/15 bg-white p-4">
                <button type="button" onClick={() => void setStatus('Approved', `/api/wave3/hr/leave/${id}/approve`, {})} className="bg-harvics-burgundy px-4 py-2 text-[10px] font-bold uppercase tracking-[0.14em] text-harvics-cream">Approve</button>
                <button type="button" onClick={() => void setStatus('Rejected', `/api/wave3/hr/leave/${id}/reject`, {})} className="border border-red-300 px-4 py-2 text-[10px] font-bold uppercase text-red-800">Reject</button>
                <button type="button" onClick={() => void setStatus('Cancelled', `/api/wave3/hr/leave/${id}/cancel`, {})} className="border border-harvics-burgundy/30 px-4 py-2 text-[10px] font-bold uppercase">Cancel</button>
              </div>
            ) : null}
            {doc.status === 'Approved' ? (
              <button type="button" onClick={() => void setStatus('Cancelled', `/api/wave3/hr/leave/${id}/cancel`, {})} className="border border-harvics-burgundy/30 px-4 py-2 text-[10px] font-bold uppercase">Cancel</button>
