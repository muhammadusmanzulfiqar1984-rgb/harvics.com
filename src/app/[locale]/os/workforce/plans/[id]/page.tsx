...
export default function DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave5/headcount-plans/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  const setStatus = async (status: string, path?: string, body?: any) => {
    try {
      setError('')
      setMessage('')
      const endpoint = path || `/api/wave5/headcount-plans/${id}/status`
  ...
  const title = doc ? (doc.department || 'Plan') : 'Document'

  return (
    <HarvicsOSShell
      title={typeof title === 'string' ? title : String(title)}
      subtitle="Module #33 — SAP+ headcount plan"
      activeDomain="hr"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Workforce', href: '/os/workforce' },
        { label: typeof title === 'string' ? title : 'Doc' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/workforce`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← Workforce workspace
        </Link>
          ...
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ['Dept', doc.department],
                ['Period', doc.period],
                ['Current FTE', doc.currentFte],
                ['Planned FTE', doc.plannedFte],
                ['Attrition %', doc.attritionPct],
                ['Hiring need', doc.hiringNeed],
                ['Notes', doc.notes || '—'],
