...
export default function Module62DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave6/events/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  const setStatus = async (status: string) => {
    try {
      setError(''); setMessage('')
      await api(`/api/wave6/events/${id}/status`, { method: 'POST', body: JSON.stringify({ status }) })
  ...
  return (
    <HarvicsOSShell
      ...
      subtitle="Module #62 — SAP+ event document"
      activeDomain="events"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Events', href: '/os/events' },
        { label: doc ? String(doc.title) : 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/events`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← Events workspace
        </Link>
        ...
        {!loading && doc ? (
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              ['Type', doc.type ?? '—'],
              ['Status', doc.status ?? '—'],
              ['Capacity', (doc._count?.registrations ?? 0) + '/' + (doc.capacity ?? '—')],
              ['Starts', doc.startsAt ? new Date(doc.startsAt).toLocaleString() : '—'],
              ['Ends', doc.endsAt ? new Date(doc.endsAt).toLocaleString() : '—']
              ].map(([k, v]) => (
