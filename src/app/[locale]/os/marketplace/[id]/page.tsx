...
export default function Module60DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave6/listings/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  const setStatus = async (status: string) => {
    try {
      setError(''); setMessage('')
      await api(`/api/wave6/listings/${id}/status`, { method: 'POST', body: JSON.stringify({ status }) })
  ...
  return (
    <HarvicsOSShell
      ...
      subtitle="Module #60 — SAP+ listing document"
      activeDomain="marketplace"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Marketplace', href: '/os/marketplace' },
        { label: doc ? String(doc.title) : 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/marketplace`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
        ...
        {!loading && doc ? (
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              ['Price', (doc.currency || '') + ' ' + (doc.price ?? '—')],
              ['Qty', doc.qtyAvailable ?? '—'],
              ['Status', doc.status ?? '—'],
