...
export default function MapPage() {
  ...
  return (
    <>
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-harvics-gold mb-2">Digital Mapping</h1>
        <p className="text-harvics-gold/90">View warehouses, retailers, and distribution network</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Warehouses */}
        <div className="bg-white border border-black200 p-6">
          <h2 className="text-xl font-semibold text-harvics-gold/90 mb-4">Warehouses</h2>
          {loading ? (
            <div className="text-center py-8">Loading...</div>
          ) : (
            <div className="space-y-3">
              {warehouses.length === 0 ? (
                <p className="text-harvics-gold/90 text-center py-4">No warehouses found</p>
              ) : (
                warehouses.map((warehouse) => (
                  <div key={warehouse.id} className="border-b border-black100 pb-3">
                    <div className="font-medium text-harvics-gold/90">{warehouse.name}</div>
                    <div className="text-sm text-harvics-gold/90">{warehouse.location}</div>
                    <div className="text-sm text-harvics-gold/90 mt-1">
        ...
        {/* Retailers */}
        <div className="bg-white border border-black200 p-6">
          <h2 className="text-xl font-semibold text-harvics-gold/90 mb-4">Retailers</h2>
          {loading ? (
            <div className="text-center py-8">Loading...</div>
          ) : (
            <div className="space-y-3">
              {retailers.length === 0 ? (
                <p className="text-harvics-gold/90 text-center py-4">No retailers found</p>
              ) : (
                retailers.map((retailer) => (
                  <div key={retailer.id} className="border-b border-black100 pb-3">
                    <div className="font-medium text-harvics-gold/90">{retailer.name}</div>
                    <div className="text-sm text-harvics-gold/90">{retailer.location}</div>
                    <div className="text-sm text-harvics-gold/90 mt-1">
      ...
      {/* Map Placeholder */}
      <div className="bg-white border border-black200 p-6">
        <h2 className="text-xl font-semibold text-harvics-gold/90 mb-4">Map View</h2>
