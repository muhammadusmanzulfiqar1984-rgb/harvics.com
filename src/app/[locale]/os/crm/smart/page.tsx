          ...
          <div className={`rounded-2xl p-5 min-w-[220px] backdrop-blur-xl ${aiHealth?.aiEnabled ? 'bg-white/5 border border-harvics-gold/35 shadow-[0_0_40px_rgba(195, 163, 94,0.1)]' : 'bg-white/5 border border-white/10'}`}>
            {aiHealth?.aiEnabled ? (
              ...
              <>
                ...
                <div className="text-[10px] text-harvics-muted">Set GROQ_API_KEY in .env.local</div>
              </>
            )}
            <button
              onClick={bulkScore}
              disabled={busy === 'bulk' || !leads.length}
              className="mt-3 w-full py-2.5 rounded-xl bg-harvics-gold hover:bg-[#b8923e] disabled:opacity-40 disabled:cursor-not-allowed text-harvics-burgundy text-[11px] font-black tracking-[0.08em] transition-colors shadow-[0_4px_14px_rgba(195, 163, 94,0.35)] flex items-center justify-center gap-2"
            >
              <Brain size={13} />
            ...
            return (
              <div key={stage} className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-3 min-h-[200px]">
                ...
                <div className="flex flex-col gap-2">
                  {sl.length === 0 && <p className="text-[11px] text-harvics-muted/50 text-center py-5 italic">empty</p>}
                  {sl.map(l => (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              ...
              <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-4">
                ...
                {selected.aiScore != null ? (
                  ...
                  <div className="text-sm text-harvics-muted mt-2">not scored yet</div>
                )}
              </div>
            </div>
              ...
              <div className="bg-harvics-gold/[0.06] border border-harvics-gold/30 rounded-2xl p-4 mb-4">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-harvics-muted mt-1">confidence score</div>
                  </div>
                </div>
              ...
              <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-4 mb-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <button
                    ...
                    className="ml-auto flex items-center gap-1.5 px-2.5 py-1 border border-harvics-gold/40 rounded-lg text-harvics-gold text-[10px] font-bold hover:bg-harvics-gold/10 transition-colors"
                  >
                    <Copy size={11} />COPY
                  </button>
                </div>
                ...
                <pre className="whitespace-pre-wrap text-[13px] text-harvics-muted font-sans leading-relaxed px-3 py-3 bg-black/20 rounded-xl">{draft.body}</pre>
              </div>
            )}
            ...
            <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-4 mb-4">
              ...
              <button
                ...
                className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-harvics-gold hover:bg-[#b8923e] disabled:opacity-40 disabled:cursor-not-allowed rounded-lg text-harvics-burgundy text-[11px] font-black tracking-[0.1em] transition-colors"
              >
                <Plus size={12} />
            ...
            <div>
              ...
              {!timeline?.activities?.length ? (
                ...
                <div className="relative pl-6">
                  <div className="absolute left-1.5 top-2 bottom-2 w-0.5 bg-gradient-to-b from-harvics-gold/50 to-transparent" />
...
function MetricCard({ label, value, sub, icon, gauge }: { label: string; value: string; sub: string; icon: React.ReactNode; gauge?: number }) {
  return (
    <div className="relative bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] rounded-2xl p-5 overflow-hidden">
      ...
      <div className="text-[11px] text-harvics-muted mt-2">{sub}</div>
    </div>
  )
}
...
function LeadCard({ lead, active, onClick, fmt, stageText, stageAccent }: {
  ...
  return (
    ...
    >
      ...
      {lead.contact && <div className="text-[10px] text-harvics-muted mt-0.5 truncate">{lead.contact}</div>}
