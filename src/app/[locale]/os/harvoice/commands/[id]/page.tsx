...
export default function Module57DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave7/voice/commands/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  return (
    <HarvicsOSShell
      title={doc ? String(doc.intent) : 'Document'}
      subtitle="Module #57 — SAP+ voice command document"
      activeDomain="harvoice"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Harvoice', href: '/os/harvoice' },
        { label: doc ? String(doc.intent) : 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/harvoice`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← Harvoice workspace
        </Link>
        ...
        {!loading && doc ? (
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
              ['Intent', doc.intent ?? '—'],
              ['Confidence', doc.confidence != null ? Math.round(doc.confidence * 100) + '%' : '—'],
              ['Status', doc.status ?? '—'],
