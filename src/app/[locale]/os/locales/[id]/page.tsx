...
export default function Module58DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/platform/locales/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  const toggle = async () => {
    try {
      ...
      setMessage(r.data?.enabled ? 'Enabled' : 'Disabled')
      await load()
  ...
  return (
    <HarvicsOSShell
      title={doc ? String(doc.code) : 'Document'}
      subtitle="Module #58 — SAP+ locale document"
      activeDomain="locales"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Locales', href: '/os/locales' },
        { label: doc ? String(doc.code) : 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/locales`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← Locales workspace
        </Link>
        ...
        {!loading && doc ? (
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              ['Enabled', doc.enabled == null ? '—' : String(doc.enabled)],
            ...
            <div className="flex flex-wrap gap-2 border border-harvics-burgundy/15 bg-harvics-cream/40 p-4">
              ...
              <button type="button" onClick={() => void toggle()} className="bg-harvics-burgundy px-4 py-2 text-[10px] font-bold uppercase tracking-[0.14em] text-harvics-cream">{doc.enabled ? 'Disable' : 'Enable'}</button>
            </div>
          </>
        ) : null}
      </div>
