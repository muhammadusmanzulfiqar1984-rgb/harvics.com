...
export default function DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/v2/legal/cases/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  const setStatus = async (status: string, path?: string, body?: any) => {
    try {
      setError('')
      setMessage('')
      const endpoint = path || `/api/v2/legal/cases/${id}/status`
  ...
  const title = doc ? (doc.caseTitle || 'Case') : 'Document'

  return (
    <HarvicsOSShell
      title={typeof title === 'string' ? title : String(title)}
      subtitle="Module #39 — SAP+ legal case"
      activeDomain="legal"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
    ...
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/legal`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← Legal workspace
        </Link>
          ...
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                ['Hearing', doc.hearingDate ? new Date(doc.hearingDate).toLocaleDateString() : '—'],
