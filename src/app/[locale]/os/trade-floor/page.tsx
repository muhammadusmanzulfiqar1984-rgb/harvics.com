  ...
  <Grid cols={3}>
    ...
    <Panel title={`ORDER BOOK — ${pick||'—'}`}>
      {!book?<div style={{padding:20,color:'#888',textAlign:'center'}}>Pick instrument →</div>:<>
        <div style={{padding:6,background:'var(--harvics-cream)',marginBottom:6,fontSize:11,color:B,fontWeight:600}}>Last: <b style={{fontSize:14}}>${book.instrument?.lastPrice}</b> · Spread: {book.spread!==null?`$${book.spread}`:'—'}</div>
    ...
    <Panel title={`RECENT TRADES (${trades.length})`} full>
      <Tbl head={['WHEN','SYMBOL','PRICE','QTY','BUYER','SELLER']}>
        {trades.map(t=><tr key={t.id} style={{borderBottom:'1px solid #3D121211'}}>
