...
export default function DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave5/perf-reviews/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  const setStatus = async (status: string, path?: string, body?: any) => {
    try {
      setError('')
      setMessage('')
      const endpoint = path || `/api/wave5/perf-reviews/${id}/status`
  ...
  const title = doc ? (doc.employeeId || 'Review') : 'Document'

  return (
    <HarvicsOSShell
      title={typeof title === 'string' ? title : String(title)}
      subtitle="Module #32 — SAP+ performance review"
      activeDomain="hr"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Performance', href: '/os/performance' },
        { label: typeof title === 'string' ? title : 'Doc' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/performance`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
