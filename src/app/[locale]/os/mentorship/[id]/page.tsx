...
export default function Module63DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/wave6/mentors/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  return (
    <HarvicsOSShell
      ...
      subtitle="Module #63 — SAP+ mentor document"
      activeDomain="mentorship"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Mentorship', href: '/os/mentorship' },
        { label: doc ? String(doc.name) : 'Document' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/mentorship`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
        ...
        {!loading && doc ? (
          <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              ['Years', doc.yearsExp ?? '—'],
              ['Rating', doc.rating ?? '—'],
              ['Accepting', doc.acceptingMentees == null ? '—' : String(doc.acceptingMentees)],
