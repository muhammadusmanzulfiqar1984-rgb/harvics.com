...
export default function P(){
  ...
  <Grid cols={2}>
    <Panel title="GENERATE PACK">
      ...
      <div style={{marginTop:10,padding:10,background:'var(--harvics-cream)',borderLeft:'4px solid #C3A35E',fontSize:11,color:'var(--harvics-burgundy)'}}>Pulls real KPIs from Orders, Invoices, Customers, Leads, Work Orders. Re-running overwrites the pack for that period.</div>
    </Panel>
    ...
    {open && <Panel title={`PACK — ${open.title}`} full>
      {(open.sections as any[]).map((s,i)=><div key={i} style={{padding:10,borderBottom:'1px solid #3D121222'}}>
        <div style={{fontWeight:700,color:'var(--harvics-burgundy)',fontSize:14}}>{s.name}</div>
        <div style={{fontSize:12,marginTop:4}}>{s.content}</div>
        {s.kpis && <div style={{marginTop:6,display:'flex',gap:16,flexWrap:'wrap'}}>{Object.entries(s.kpis).map(([k,v]:any)=><div key={k} style={{padding:'4px 12px',background:'var(--harvics-cream)',border:'1px solid #C3A35E'}}>
          <div style={{fontSize:10,color:'#666',fontWeight:600}}>{k.toUpperCase()}</div>
          <div style={{fontSize:18,fontWeight:700,color:'var(--harvics-burgundy)'}}>{typeof v==='number'?v.toLocaleString():v}</div>
        </div>)}</div>}
      </div>)}
    </Panel>}
  </Grid></Page>
