...
export default function Module72DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/executive/snapshots/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  return (
    <HarvicsOSShell
      title={doc ? String(doc.period) : 'Document'}
      subtitle="Module #72 — SAP+ board snapshot document"
      activeDomain="executive"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Executive', href: '/os/executive' },
        { label: doc ? String(doc.period) : 'Document' },
      ]}
    >
