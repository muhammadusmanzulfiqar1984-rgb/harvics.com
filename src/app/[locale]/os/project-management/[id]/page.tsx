...
/**
 * Project document — Module #45 (SAP-style).
 */
...
export default function Module45DocumentPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('')
    try {
      const r = await api(`/api/v2/projects/${id}`)
      setDoc(r.data)
    } catch (e: any) {
  ...
  return (
    <HarvicsOSShell
      title={doc?.code || 'Project'}
      subtitle="Module #45 — SAP+ project document"
      activeDomain="projects"
      breadcrumbs={[
        { label: 'OS', href: '/os' },
        { label: 'Projects', href: '/os/project-management' },
        { label: doc?.code || 'Project' },
      ]}
    >
      <div className="space-y-5 p-5 text-harvics-burgundy">
        <Link href={`/${locale}/os/project-management`} className="text-[10px] font-bold uppercase tracking-[0.14em] underline">
          ← Projects workspace
        </Link>
