...
export default async function LaPresLoungePage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  const { locale } = await params
  const area = presentationAreas.lounge
  const decks = getPresentationsByCategory('lounge')

  return (
    <PresentationZoneGuard mode={{ type: 'zone', zone: 'lounge' }}>
