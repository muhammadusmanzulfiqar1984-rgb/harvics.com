...
export default function DistributorLoginForm() {
    ...
    try {
      ...
      if (response.error) {
        ...
        if ((formData.username === 'admin' && formData.password === 'admin') || 
          ...
          // Store demo token for distributor
          if (typeof window !== 'undefined') {
            localStorage.setItem('auth_token', 'demo-token-distributor')
            localStorage.setItem('user_type', 'distributor')
            localStorage.setItem('user_scope', JSON.stringify({
              userId: formData.username === 'admin' ? 'admin' : 'distributor_user',
              role: 'distributor',
              countries: ['AE'],
        ...
        if (typeof window !== 'undefined') {
          ...
          localStorage.setItem('user_type', response.data.user?.scope?.role || 'distributor')
        }
        routeByRole(response.data.user?.scope?.role)
      }
    } catch (err) {
      // Fallback to demo mode for distributor
      if ((formData.username === 'admin' && formData.password === 'admin') || 
          (formData.username === 'distributor_user' && formData.password === 'password')) {
        if (typeof window !== 'undefined') {
          localStorage.setItem('auth_token', 'demo-token-distributor')
  ...
  const routeByRole = (role?: string) => {
    ...
    // Default fallback to distributor portal
    router.push(`/${locale}/distributor-portal`)
  }
